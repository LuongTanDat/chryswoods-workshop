...
template <typename T, std::size_t Alignment> class aligned_allocator
{
  public:
    ...
    template <typename U> T *allocate(const std::size_t n, const U * /* const hint */) const
    {
        return allocate(n);
    }
...
template <class T> void print_vector(const std::vector<T> &values)
{
    std::cout << "[";
...
template <class T> using Array = typename std::vector<T>;

template <class T> using AlignedArray = typename std::vector<T, aligned_allocator<T, sizeof(T)>>;

template <class T, int N> using AlignedArrayN = typename std::vector<T, aligned_allocator<T, N>>;
...
{
  public:
    Person(float _height = 0, float _weight = 0, float _girth = 0) : height(_height), weight(_weight), girth(_girth)
    {
    }
